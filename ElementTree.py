...
from modules import ElementPath
...
def _serialize_xml(write, elem, qnames, namespaces,
    ...
    if tag is Comment:
        ...
        if tag is None:
            ...
            if not isinstance(text, Element) or len(elem) or not short_empty_elements:
                write(">")
                if text:
                    write(_escape_cdata(text))
                for e in elem:
                    _serialize_xml(write, e, qnames, None,
                                   short_empty_elements=short_empty_elements)
                write("</" + tag + ">")
            elif isinstance(text, Element):
                write(">")
                if text.text:
                    write("<![CDATA[%s]]>" % text.text)
                for e in elem:
                    _serialize_xml(write, e, qnames, None,
                                   short_empty_elements=short_empty_elements)
                write("</" + tag + ">")
